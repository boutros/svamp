...
(defn multi-uri-input
  "Component: Input element where value must be a URI and where predicates can
  be chosen from a dropdown list. Includes search-bar."
  [value owner]
  (reify
    om/IInitState
    (init-state [_]
      ...
      (let [chosen-chan (om/get-state owner :chosen-chan)]
        (go
          (loop []
            ...
            (recur)))))
    om/IRenderState
    (render-state [_ {:keys [chosen chosen-chan delete-chan predicates]}] ;; TODO delete-chan
      (dom/div #js {:className "relative row"}
        (dom/div #js {:className "column half"}
          (dom/input #js {:value (:value value) :disabled true :type "text" :title chosen})
          (when (seq (:value value))
            (dom/span #js {:className "delete mrgh" :onClick #(om/update! value :value nil)} "x"))
          (apply dom/select nil
            (map (fn [p] (dom/option #js {:value (:predicate p)} (:label p))) predicates)))
        (om/build uri-search-bar value {:init-state {:chosen-chan chosen-chan}})))))
  ...
  (reify
    ...
    (render [_]
      (dom/div nil
        ...
        (apply dom/div nil
          (om/build-all single-view (:elements group)))))))
